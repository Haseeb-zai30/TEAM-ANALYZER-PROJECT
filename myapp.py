...
def generate_analysis(formation, team):
    """Generates tactical analysis using the LLM (Claude-3 Haiku)."""
...
/* Light green background for the main app container */
.stApp {{ background: #e8f5e9; }} 

/* Main headers (h1, h2, h3) - Targeted for BLACK text */
h1, h2, h3 {{ 
    color: #000 !important; /* Deep black for main titles and section headers */
}}

/* Button Styling (Kept Gold/Black Theme) */
.stButton>button {{ 
...
/* Streamlit Header text alignment (e.g., 'The Pitch', 'Tactical Analysis') */
.main-header {{ 
    color: #000; /* Ensuring Streamlit generated headers are black */
}}
...
/* Pitch Container: Uses the remote image URL */
.pitch-container {{
    position: relative;
    width: 100%;
...
/* Player Marker Styling (Circle + Name/Image) */
.player-marker {{
    position: absolute;
    width: 100px; 
    height: auto;
    text-align: center;
    transform: translate(-50%, -50%); /* Centers the marker relative to its position */
    z-index: 2; 
}}

/* Player Image (Profile photo with Gold border) */
...
/* Player Name Tag: TEXT IS BLACK */
.player-marker .name-tag {{
    background-color: white;
    color: #000; /* <--- Explicitly set to BLACK TEXT */
    font-weight: bold;
    padding: 2px 6px;
    border-radius: 4px;
    margin-top: 5px;
